...
| `deviceId` | 17 ký tự `dev-` + 12 hex | `hex(anchor)[:12]` | Công khai | RAM (suy ra mỗi boot) + nhãn/QR + Supabase (phase 2) | Định danh MQTT: username `device-{deviceId}`, topic `devices/{deviceId}/#` |
| `apSSID` | ≤ 32 ký tự `myhome-<model>-XXXX` (model = pump/switch/..., XXXX = 4 hex) | `"myhome-" + profileName() + "-" + hex(anchor)[:4]` | Công khai | RAM (suy ra mỗi boot) | SSID AP pairing portal (open, không mật khẩu) — app lọc prefix `myhome-` + model để nhận biết loại thiết bị |
...
| `controlKey` | 32B (hex 64) | **App sinh ra**, gửi trong lệnh `pair`; hoặc `provision` (factory) ghi đè; tạm thời thiết bị tự sinh lúc boot đầu | Thiết bị + app đã pair (app lưu vào Supabase) | KV `ident` (mã hóa AES-GCM) | **Khóa HMAC** envelope lệnh (phase 2: `hmac = HMAC-SHA256(controlKey, seq\|ts\|payload)`) |
...
| Gửi lệnh giả mạo / replay | HMAC envelope + `seq`/`ts` | phase 2 |
...
| WiFi/mqtt password nằm plaintext trong KV config (không mã hóa theo anchor) | Trung bình | Clone lấy được wifi creds nhà nạn nhân (nhưng cloud vẫn không dùng được). Phase sau: cân nhắc mã hóa config hoặc chấp nhận (firmware $0) |
...
| ESP32 dùng MAC eFuse 6B (lệch plan 8B `esp_efuse_get_chip_serial_number` — API không tồn tại trong IDF 4.4/Arduino core 3.x) | Chấp nhận | Độ dài khác nhau giữa chip, mọi suy diễn (deviceId, key) đều theo `_anchorLen` thực tế |
...
|---|---|
| `deviceSecret` | Password MQTT: username `device-{deviceId}` / password `deviceSecret`; dán vào HiveMQ console 1 lần lúc factory provisioning |
| `controlKey` | Envelope lệnh: `hmac = HMAC-SHA256(controlKey, seq\|ts\|payload)`; thiết bị kiểm tra `seq` tăng + `ts` trong ±60s + HMAC đúng (xem ECOSYSTEM_PLAN §3.2) |
