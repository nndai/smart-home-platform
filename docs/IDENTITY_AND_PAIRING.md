...
| `controlKey` | 32B (hex 64) | **App sinh ra**, gửi trong lệnh `pair`; hoặc `provision` (factory) ghi đè; thiết bị tự sinh lúc boot đầu (trước khi pair) | Thiết bị + app đã pair (app lưu vào Supabase) | KV `ident` (mã hóa AES-GCM, key = SHA-256(deviceId + FW_SECRET)) | **Khóa HMAC** envelope lệnh: `hmac = HMAC-SHA256(controlKey, seq\|ts\|cmd\|payload)` |
